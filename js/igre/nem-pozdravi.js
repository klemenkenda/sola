/* nem-pozdravi.js - Nemščina: poveži oblačke s pozdravi (vlečenje z miško ali klik) */
(function (global) {
  'use strict';

  var O = global.Osnova;

  /* Osnovni pozdravi za prvo uro nemščine. Guten Tag in Auf Wiedersehen sta vljudna,
     Hallo in Tschüss prijateljska. "izgovor" je približek, zapisan po slovensko -
     otrok nemškega zapisa še ne zna prebrati. */
  var PARI = [
    { nem: 'Guten Morgen', slo: 'Dobro jutro', izgovor: 'gúten mórgen' },
    { nem: 'Guten Tag', slo: 'Dober dan', izgovor: 'gúten tág' },
    { nem: 'Guten Abend', slo: 'Dober večer', izgovor: 'gúten ábent' },
    { nem: 'Gute Nacht', slo: 'Lahko noč', izgovor: 'gúte náht' },
    { nem: 'Hallo', slo: 'Zdravo', izgovor: 'hálo' },
    { nem: 'Tschüss', slo: 'Adijo', izgovor: 'čüs' },
    { nem: 'Auf Wiedersehen', slo: 'Nasvidenje', izgovor: 'auf vídarzejen' },
    { nem: 'Bis später', slo: 'Se vidiva pozneje', izgovor: 'bis špéter' },
    { nem: 'Wie geht\'s?', slo: 'Kako si?', izgovor: 'vi géts' }
  ];

  /* Nemško besedilo mora prebrati nemški glas - angleški bi "Guten Morgen"
     izgovoril po angleško. */
  var JEZIK = 'de-DE';

  function izgovoriNem(besedilo) {
    global.Ucinki.izgovori(besedilo, JEZIK);
  }
  ...
  function zazeni(posoda, ctx) {
    ...
    function pokaziUvod() {
      posoda.innerHTML =
        ...
        '<div class="oblacek">Učimo se <b>nemške pozdrave</b>. Klikni na kartico, ' +
        ...
        '<p class="namig-opis"><b>Guten Tag</b> in <b>Auf Wiedersehen</b> sta vljudna — ' +
        'za odrasle in učitelje, <b>Hallo</b> in <b>Tschüss</b> pa prijateljska, za sošolce. ' +
        'V nemščini se vsi samostalniki pišejo z <b>veliko začetnico</b>: ' +
        'Morgen, Tag, Abend, Nacht.</p>' +
      ...
      PARI.forEach(function (p) {
        ...
        el.innerHTML = '<b>' + p.nem + '</b><span>' + p.slo + '</span>' +
          '<small>izgovori: ' + p.izgovor + '</small>';
    ...
    function zacniKrog() {
      ...
      /* Vse kartice postavimo v krog okoli sove - nemške in slovenske pomešano,
         v dveh polmerih, da se sosedi ne prekrivajo. */
      function izrisiKrog() {
        var vnosi = [];
        PARI.forEach(function (p, i) {
          vnosi.push({ stran: 'nem', indeks: i });
      ...
      function koncajVlecenje(e) {
        ...
        var ciljStran = cilj.classList.contains('nem') ? 'nem' : 'slo';
      ...
      function pocistiIzbrano() {
        ...
        stanje.izbrana = { nem: null, slo: null };
      }
      ...
      function osveziPovezave() {
        ...
        stanje.povezani.forEach(function (indeks) {
          var a = kartica('nem', indeks), s = kartica('slo', indeks);
          if (!a || !s) return;
      ...
      function klikni(stran, indeks, gumb) {
        ...
        global.Ucinki.zvok.klik();
        if (stran === 'nem') izgovoriNem(PARI[indeks].nem);

        if (stanje.izbrana.nem && stanje.izbrana.slo) preveri();
      }

      /* ---------- preverjanje para ---------- */

      function preveri() {
        var a = stanje.izbrana.nem;
        ...
        if (a.indeks === s.indeks) {
          ...
          odziv.textContent = O.pohvala() + ' ' + PARI[a.indeks].nem + ' = ' +
      ...
      function pokaziNamig() {
        ...
        var odziv = document.getElementById('odziv');
        var izbran = stanje.izbrana.nem || stanje.izbrana.slo;
        ...
        var iskana = stanje.izbrana.nem ? 'slo' : 'nem';
        ...
        odziv.className = 'odziv ok';
        odziv.textContent = PARI[indeks].nem + ' = ' + PARI[indeks].slo +
          ' (' + PARI[indeks].izgovor + ')';
        global.Ucinki.zvok.klik();
        osveziStanje();
      }
      ...
      function pokaziKonec() {
        ...
        var napake = Object.keys(stanje.zgreseni).map(function (i) {
          return { vprasanje: PARI[i].nem, odgovor: PARI[i].slo };
        });
        ...
        O.koncniZaslon(posoda, ctx, {
          ...
          sporocila: {
            tri: 'Vrhunsko! Nemške pozdrave že obvladaš!',
  ...
  global.Igre.registriraj({
    ...
    opis: 'Povleci črto med nemškim pozdravom in slovenskim prevodom.',
    ikona: '👋',
    zazeni: zazeni
  });
})(window);
